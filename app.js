...
import { normalizeToJpeg, normalizeFiles, blobToImageUrl } from './image-util.js';
...
function initSpeech() {
  ...
  recognition.onstart = () => {
    ...
    els.voiceHint.textContent = '正在听…请说「1201」或「阳光花园1201」';
  ...
  recognition.onend = async () => {
    ...
    if (voiceTranscript && pendingFiles.length) {
      await handleVoiceCommand(voiceTranscript);
      voiceTranscript = '';
      return;
    }
    voiceTranscript = '';
...
function handlePhotoPick(fileList, isCamera) {
  const raw = [...fileList].filter((f) => f.type.startsWith('image/') || f.name?.match(/\.(jpg|jpeg|png|webp|heic|heif)$/i));
  if (!raw.length) {
    showToast('未选到图片', 'error');
    return;
  }

  normalizeFiles(raw).then((normalized) => {
    ...
    if (isCamera && normalized.length === 1) {
      openPhotoEditor(normalized[0], (edited) => {
        ...
        showToast('照片已留存，说话或点「手动选房源」', 'success');
      });
    } else {
      appendPendingFiles(normalized);
      showToast(`已添加 ${normalized.length} 张，可继续添加或说话归档`, 'success');
    }
  });
}
...
function updatePendingUI() {
  ...
  if (n > 0) {
    ...
    els.previewImg.src = blobToImageUrl(pendingFiles[0]);
    if (countEl) {
    ...
    els.voiceHint.textContent = `已选 ${n} 张 → 点🎤说房号，或点「手动选房源」`;
  } else {
    clearPendingFiles();
  }
}

function openPickPropertyDialog() {
  ...
  $('#pick-property-hint').textContent = `已选 ${pendingFiles.length} 张照片，选房源后确认`;
...
function clearPendingFiles() {
  ...
  els.voiceHint.textContent = '先从相册选图，再说话或手动选房源';
}
...
async function handleVoiceCommand(transcript) {
  ...
  if (!result) {
    showToast(`没匹配到房源，请点「手动选房源」`, 'error');
    openPickPropertyDialog();
    return;
  }
...
async function renderAlbum() {
  ...
  els.photoGrid.innerHTML = photos.map((photo) => {
    const url = blobToImageUrl(photo.blob);
    if (url) photoObjectUrls.push(url);
    const tagHtml = photo.tag ? `<span class="tag">${escapeHtml(photo.tag)}</span>` : '';
    return `
      <div class="photo-card" data-id="${photo.id}">
        <img src="${url}" alt="" loading="lazy" onerror="this.style.background='#333'">
        ${tagHtml}
